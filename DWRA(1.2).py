...
class UserInfo:
    ...
    def printinfo(self):
        ...
        if self.passtype == 0:
            print(f'You want to go to {parks[self.park]} on {str(self.month + 1)}-{str(self.day)}-{str(self.year)} using {tickettypes[self.tickettype - 1]}.\nThe tabID is --> {str(tabID)}.')
        else:
            print(f'You want to go to {parks[self.park]} on {str(self.month + 1)}-{str(self.day)}-{str(self.year)} using your {passes[self.passtype]}.\nThe tabID is --> {str(tabID)}.')
...
def getdate():
    ...
    while True: 
        ...
        if month == 1 and day == 29:
            ...
            print(f"[INVALID INPUT] -- {month}, {day}, {year} IS NOT A VALID DATE")                
def getpark():
...
def getavail(info):
    ...
    driver.switch_to.window(str(info.tabID))
    ...
    if info.passtype == 0: #non-passholder 
        ...
        if available == "available":
            print(f"{str(parks[info.park])} IS available on {str(info.month + 1)}/{str(info.day)}/{str(info.year)} with {str(tickettypes[info.tickettype - 1])}! --- {str(time.ctime())}")
            return 1
        else:
            print(f"{str(parks[info.park])} IS NOT available on {str(info.month + 1)}/{str(info.day)}/{str(info.year)} with {str(tickettypes[info.tickettype - 1])}! --- {str(time.ctime())}")
            return 0
    else: # passholder
        ...
        if info.month < 10:
            formattedmonth = str(f"0{info.month + 1}") #adds a '0' if necessary to month
        else:
            formattedmonth = str(info.month + 1)
        ...
        if available == "all green":
            print(f"{str(parks[info.park])} IS available on {str(info.month + 1)}/{str(info.day)}/{str(info.year)} for the {passes[info.passtype]}! --- {str(time.ctime())}")
            return 1
        elif available == "no-reservations":
            print(f"{str(parks[info.park])} is NOT available on {str(info.month + 1)}/{str(info.day)}/{str(info.year)} for the {passes[info.passtype]}. --- {str(time.ctime())}")
            return 0
        elif available == "blocked":
            print(f"The {passes[info.passtype]} is BLOCKED OUT for {str(info.month + 1)}/{str(info.day)}/{str(info.year)}. Searches will stop for this entry. --- {str(time.ctime())}")
            return 2
        else:
            print("[ERROR]")
def setuptab(info):
...
while True:
    ...
    wait.until(expected_conditions.number_of_windows_to_be(numofdesireddates))
